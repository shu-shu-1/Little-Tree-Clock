...
class PluginManager(QObject):
    ...
    def all_known_plugins(self) -> List[tuple]:
        """返回所有已发现插件（含已禁用）的信息三元组列表：
        ``(meta: PluginMeta, enabled: bool, error: str | None)``
        """
        result: List[tuple] = []
        seen: set[str] = set()
        for entry in self._entries.values():
            result.append((entry.meta, entry.enabled, entry.error))
            seen.add(entry.meta.id)
        for pid, meta in self._disabled_metas.items():
            if pid not in seen:
                result.append((meta, False, None))
        return result
    ...
    def _load_from_path(self, path: Path) -> None:
        ...
        if is_pkg:
            plugin_name = manifest.name if manifest else path.name
            plugin_id   = manifest.id   if manifest else path.stem
            ...
            if manifest and manifest.permissions:
                for perm_key in manifest.permissions:
                    ...
                    if not allowed:
                        logger.info("插件 {} 系统权限 {} 被拒绝，跳过加载", path.name, perm_key)
                        if manifest:
                            self._disabled_metas[manifest.id] = manifest
                        self.pluginError.emit(
                            path.stem,
                            f"系统权限被拒绝: {PERMISSION_NAMES.get(perm_key, perm_key)}",
                        )
                        return
            ...
            if missing:
                ...
                if not allowed:
                    logger.info("插件 {} 依赖安装已被拒绝，跳过加载", path.name)
                    if manifest:
                        self._disabled_metas[manifest.id] = manifest
                    self.pluginError.emit(
                        path.stem,
                        f"依赖安装被拒绝: {', '.join(missing)}",
                    )
                    return
            ...
            if failed_deps:
                ...
                self.pluginError.emit(
                    path.stem,
                    f"依赖安装失败: {', '.join(failed_deps)}，请检查网络或手动安装",
                )
                return
...
class _SharedAPIAdapter(PluginAPI):
    ...
    def register_trigger(self, trigger_id: str, handler=None, *, name: str = "", description: str = ""):
        self._local.register_trigger(trigger_id, handler, name=name, description=description)
        self._shared.register_trigger(trigger_id, handler, name=name, description=description)
