...
class WorldZoneStore:
    ...
    def _load(self):
        ...
        if self.__class__._cache_zones is not None and self.__class__._cache_mtime_ns == mtime_ns:
        ...
        if data is None:
            # 首次运行：写入预设时区
            self._zones = [WorldZone(label=label, timezone=tz) for label, tz in PRESET_TIMEZONES[:5]]
            self._save()
