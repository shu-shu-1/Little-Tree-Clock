...
class FeatureStats:
    ...
    def effective_visits(self, now: float | None = None) -> float:
        ...
        旧数据无 freq_mass 时按终身 visit_count 从 last_visit 衰减回退。
        """
...
class RecommendationService(QObject):
    ...
    def on_session_end(self, feature: str) -> None:
        ...
        if t0 is not None and st is not None:
            ...
            self._save_debounced()
            logger.info(
                "[推荐] 会话结束：feature='{}', elapsed_ms={}, total_session_ms={}",
                feature,
                elapsed_ms,
                st.total_session_ms,
            )
            ...
            logger.debug(
                "[推荐] 会话结束跳过：feature='{}', has_start={}, has_stats={}", feature, t0 is not None, st is not None
            )
    ...
    def _rank_features(
        ...
        if explore:
            results = [
                (
                    fid,
                    score + self._explore_noise(fid) if fid not in active else score,
                )
    ...
    def debug_rows(self) -> list[tuple[str, str]]:
        ...
        for fid in feature_ids:
            ...
            if st is None:
                ...
                continue
            last_str = "从未" if st.last_visit == 0 else datetime.fromtimestamp(st.last_visit).strftime("%m-%d %H:%M")
            total_min = st.total_session_ms / 60_000
            rows.append(
                (
    ...
    def _save(self) -> None:
        data = {
            ...
            "feature_labels": {
                fid: label for fid, label in self._feature_labels.items() if fid in self._stats and label
            },
        }
