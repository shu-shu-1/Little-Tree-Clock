...
# widget_colors / is_widget_dark 是渲染热路径（paintEvent 与每秒 refresh 都会调用），
# 结果只取决于画布级自定义颜色、全局全屏主题与系统/应用主题；
# 因此按 zone_id 缓存，并在上述任一输入变化时显式失效。
...
def invalidate_widget_color_cache(zone_id: str | None = None) -> None:
    """画布自定义颜色或全局主题变更后调用；zone_id 为 None 时清空全部缓存。"""
    if zone_id is None:
        _dark_cache.clear()
        _colors_cache.clear()
    else:
...
def widget_colors(zone_id: str | None = None) -> dict[str, str]:
    """返回当前主题下的画布组件配色字典；提供 zone_id 时叠加画布级自定义颜色。"""
