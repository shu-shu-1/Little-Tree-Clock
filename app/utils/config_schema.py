...
def _validate_position(value: Any) -> Any:
    valid_positions = {"top-left", "top-center", "top-right", "bottom-left", "bottom-center", "bottom-right", "center"}
...
def load_config_with_schema(
    path: str, schema: Schema | None = None, default: dict | None = None, log_name: str = ""
) -> dict:
    p = Path(path)
    if not p.exists():
