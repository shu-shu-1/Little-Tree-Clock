...
def _validate_manifest_data(
    ...
    *,
    ...
    _validate_i18n_field(data, "name_i18n", result, context_label=context_label)
    _validate_i18n_field(data, "description_i18n", result, context_label=context_label)

    name_value = data.get("name")
    if isinstance(name_value, dict):
        _validate_i18n_value(name_value, "name", result, context_label=context_label)

    description_value = data.get("description")
    if isinstance(description_value, dict):
        _validate_i18n_value(description_value, "description", result, context_label=context_label)

    homepage = data.get("homepage")
    if homepage is not None:
        homepage_text = str(homepage).strip()
        if not isinstance(homepage, str):
            _error(result, f"{context_label}: field 'homepage' must be a string.")
        elif homepage_text and "://" not in homepage_text:
            _warn(result, f"{context_label}: 'homepage' does not look like a URL: {homepage_text}")

    min_host_version = data.get("min_host_version")
    if min_host_version is not None:
        mhv_text = str(min_host_version).strip()
        if not isinstance(min_host_version, str):
            _error(result, f"{context_label}: field 'min_host_version' must be a string.")
        elif mhv_text and not _SEMVER_RE.match(mhv_text):
            _warn(result, f"{context_label}: 'min_host_version' should follow semver (e.g. '0.1.0'): {mhv_text}")
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    homepage = str(args.homepage or "").strip()
    min_host_version = str(args.min_host_version or "").strip()
    ...
    if min_host_version:
        if not _SEMVER_RE.match(min_host_version):
            _print_failure(f"min_host_version must follow semver (e.g. '0.1.0'): {min_host_version}")
            return 1
...
def build_parser() -> argparse.ArgumentParser:
    ...
    init_parser.add_argument(
        "--min-host-version",
        default="",
        help="Minimum required host version (semver, e.g. '0.10.3')",
    )
    init_parser.add_argument(
        "--description",
